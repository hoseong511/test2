...
public class HO2 extends Player{
	...
	public void underAttack(Player attacker) {
		...
		if (this.save_hp < attacker.hp && this.damage < attacker.damage) {
			...
			this.damage = attacker.damage; // 상대 데미지를 내 데미지로 만듦
			System.out.println("상대가 "+ this.name + "의 HP보다 쎄므로 상대의 HP와 데미지를 복사했습니다");
...
//			else if (this.hp == 0 && (this.damage < attacker.damage) && (attacker.hp > this.hp)) {
